...
def load_data():
    ...
    dataset['date'] = pd.DatetimeIndex(dataset['START_DATE']).date
    dataset['time'] = pd.DatetimeIndex(dataset['START_DATE']).hour
    dataset['day-night'] = pd.cut(dataset['time'], bins=[0, 10, 15, 19, 24], labels=['Morning', 'Afternoon', 'Evening', 'Night'], include_lowest=True)
    dataset.dropna(inplace=True)
    dataset.drop_duplicates(inplace=True)
    return dataset
...
# Show code section
with st.expander("📜 Show Data Processing Code"):
    st.code("Code already shown above (omitted for brevity)", language='python')
...
if st.button("▶️ Run Analysis"):
    ...
    sns.countplot(data=df, x='day-night', ax=ax2)
    ax2.set_title("Rides by Time of Day")
    st.pyplot(fig2)
    ...
    ax4.set_xlabel("Month")
    ax4.set_ylabel("Max Miles")
    ax4.set_title("Monthly Max Miles Traveled")
    st.pyplot(fig4)
    ...
    ax5.set_ylabel("Ride Count")
    ax5.set_title("Rides per Day of the Week")
    st.pyplot(fig5)
    ...
    sns.boxplot(x=df['MILES'], ax=ax6)
    ax6.set_title("Boxplot of All Rides")
    st.pyplot(fig6)
